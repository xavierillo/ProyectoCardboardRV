...
/// </remarks>
public class CardboardReticlePointerV2 : MonoBehaviour
{
    /// <summary>
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, _RETICLE_MAX_DISTANCE)) // si el rayo choca con algo
        {

            // GameObject detected in front of the camera.
            // GameObject detectado frente a la camara.
            if (_gazedAtObject != hit.transform.gameObject) // si ha cambiado el objeto
            {
                // New GameObject.
                if (IsInteractive(_gazedAtObject)) // SI estaba mirando a otro objeto interactivo
                {
                    _gazedAtObject?.SendMessage("OnPointerExit", SendMessageOptions.DontRequireReceiver);
                    // _gazedAtObject?.SendMessage("OnPointerExit"); // envia el mensaje de salida
                }

                _gazedAtObject = hit.transform.gameObject; // actualiza el objeto al que mira

                if (IsInteractive(_gazedAtObject)) // si el nuevo objeto es interactivo
                {
                    _gazedAtObject.SendMessage("OnPointerEnter", SendMessageOptions.DontRequireReceiver);
                    // _gazedAtObject.SendMessage("OnPointerEnter"); // envia el mensaje de entrada
                }
            }

            SetParams(hit.distance, IsInteractive(_gazedAtObject)); // actualiza los parametros del reticulo
        }
